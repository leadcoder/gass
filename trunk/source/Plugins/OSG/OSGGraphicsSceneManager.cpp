...
//move camera to spawn position
		boost::shared_ptr<Message> pos_msg(new Message(ScenarioScene::OBJECT_MESSAGE_POSITION,(int) this));
	...
	void OSGGraphicsSceneManager::OnLoadGameObject(MessagePtr message)
	{
		...
		assert(obj);
		boost::shared_ptr<Message> gfx_msg(new Message(ScenarioScene::SM_MESSAGE_LOAD_GFX_COMPONENTS,(int) this));
