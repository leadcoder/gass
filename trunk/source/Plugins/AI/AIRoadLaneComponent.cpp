...
namespace GASS
{
	...
	LaneObject* AIRoadLaneComponent::GetClosest(LaneObject* source)
	{
		tbb::spin_mutex::scoped_lock lock(m_LaneMutex);
		
		LaneObject* closest = NULL;
		double min_dist = 0;
		for(size_t i = 0 ; i < m_LaneObjects.size() ; i++)
		{
			if(m_LaneObjects.at(i) != source)
			{
				if(source->m_Distance < m_LaneObjects.at(i)->m_Distance)
				{
					double dist = m_LaneObjects.at(i)->m_Distance - source->m_Distance;
					if(closest == NULL || dist < min_dist)
					{
						closest = m_LaneObjects.at(i);
						min_dist = dist; 
					}
				}
			}
