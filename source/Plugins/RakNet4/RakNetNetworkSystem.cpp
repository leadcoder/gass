...
#include "Plugins/RakNet4/RakNetChildReplica.h"
#include "Plugins/RakNet4/RakNetInputTransferComponent.h"
...
namespace GASS
{
	...
	void RakNetNetworkSystem::Init()
	{
		...
		ARPC_REGISTER_CPP_FUNCTION4(GetRPC(), "RakNetBaseReplica::RemoteInput", int, RakNetBaseReplica, RemoteInput, RakNet::SystemAddress input_source, int controller, float value,RakNet::AutoRPC* networkCaller);
	...
	void RakNetNetworkSystem::OnTimeOfDay(TimeOfDayRequestPtr message)
	{
		if(m_IsServer && m_RakPeer)
		{
			...
			m_RakPeer->Send(&out, HIGH_PRIORITY, RELIABLE_ORDERED,0,RakNet::UNASSIGNED_SYSTEM_ADDRESS,true);
		}
	}
	...
	ReplicaReturnResult RakNetNetworkSystem::ReceiveConstruction(RakNet::BitStream *inBitStream, RakNet::Time timestamp, NetworkID networkID, NetworkIDObject *existingObject, RakNet::SystemAddress senderId, ReplicaManager *caller)
	{
		char output[255];
		...
		while(p)
		{
			if (p->data[0]==ID_DISCONNECTION_NOTIFICATION || p->data[0]==ID_CONNECTION_LOST)
			{
				...
				for (int index=0; index < m_ReplicaManager->GetReplicaCount(); index++)
				{
					...
					{
					m_ReplicaManager->Destruct(base->GetReplica(), RakNet::UNASSIGNED_SYSTEM_ADDRESS, true); //Send the destruct message to all
					delete base;
					break;
					}*/
				}
			...
			else if (p->data[0] == ID_NEW_INCOMING_CONNECTION || p->data[0]==ID_CONNECTION_REQUEST_ACCEPTED)
			{
				...
				//if (NetworkIDGenerator::GetExternalPlayerID()==RakNet::UNASSIGNED_SYSTEM_ADDRESS)
	...
	void RakNetNetworkSystem::UpdateClient(double /*delta*/)
	{
		...
		while(p)
		{
			...
			if (p->data[0]==ID_PONG)
			{
				...
				RakNet::BitStream pong( p->data+1, sizeof(RakNet::Time), false);
				pong.Read(time);
				dataLength = p->length - sizeof(unsigned char) - sizeof(RakNet::Time);
	...
	if(net_obj->AllowRemoteOwner() && m_RemoteOwnerId != RakNet::UNASSIGNED_SYSTEM_ADDRESS)
	{
