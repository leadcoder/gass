...
/*	int RakNetBaseReplica::EnterVehicle(const char *client_address, RakNet::AutoRPC* networkCaller) 
	...
	/*R4 int RakNetBaseReplica::RemoteMessageWithData(const char *message, const char *data, RakNet::AutoRPC* networkCaller)
	{
		if(m_Owner)
		{
			RakNetNetworkSystemPtr raknet = SimEngine::Get().GetSimSystemManager()->GetFirstSystemByClass<RakNetNetworkSystem>();
	...
	/*int RakNetBaseReplica::RemoteMessage(const char *client_address, const char *message , RakNet::AutoRPC* networkCaller) 
	{
		//if(m_Owner)
		{
			...
			{
				std::string my_addres(raknet->GetRakPeer()->GetExternalID(RakNet::UNASSIGNED_SYSTEM_ADDRESS).ToString());
