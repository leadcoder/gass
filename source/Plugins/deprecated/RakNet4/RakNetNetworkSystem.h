...
#define MAX_PEERS 64
	...
	class RakNetNetworkSystem  : public Reflection<RakNetNetworkSystem, SimSystem>//R4 , RakNet::ReceiveConstructionInterface
	{
	...
	public:
		...
		RakNet::NetworkIDManager* GetNetworkIDManager() const{return m_NetworkIDManager;}
		//helpers
		...
		RakNetChildReplica* FindReplica(const RakNet::NetworkID &part_of_network_id,int part_id);
		ScenePtr GetScene() {return ScenePtr(m_Scene);}
		//R4 RakNet::AutoRPC* GetRPC() {return  &m_AutoRPC;}
	...
	private:
		...
		//R4 ReplicaReturnResult ReceiveConstruction(RakNet::BitStream *inBitStream, RakNet::Time timestamp, NetworkID networkID, NetworkIDObject *existingObject, RakNet::SystemAddress senderId, ReplicaManager *caller);
		...
		RakNet::SystemAddress m_RemoteOwnerId;
		...
		//R4 RakNet::AutoRPC m_AutoRPC;
		double m_InterpolationLag;
		double m_LocationSendFrequency;
		RakNet::PacketLogger m_Logger;
	};
