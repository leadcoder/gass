...
namespace GASS
{
	RakNetChildReplica::RakNetChildReplica(RakNet::ReplicaManager3* manager): m_Manager(manager)
	{
		m_OwnerSystemAddress = RakNet::UNASSIGNED_SYSTEM_ADDRESS;
		m_AllowRemoteOwner = false;
	}

	RakNetChildReplica::~RakNetChildReplica()
	{
		m_Manager->Destruct(this, RakNet::UNASSIGNED_SYSTEM_ADDRESS, true); // Forward the destruct message to all other systems but the sender
...
//		m_TemplateName = object->GetTemplateName();
		...
		if (raknet->IsServer())
		{
			m_Manager->Construct(this, false, RakNet::UNASSIGNED_SYSTEM_ADDRESS, true);
	...
	void RakNetChildReplica::RemoteInit(RakNet::BitStream *inBitStream, RakNet::Time timestamp, NetworkID networkID, RakNet::SystemAddress senderId)
	{
		RakNetNetworkSystemPtr raknet = SimEngine::Get().GetSimSystemManager()->GetFirstSystemByClass<RakNetNetworkSystem>();
	...
	ReplicaReturnResult RakNetChildReplica::SendConstruction( RakNet::Time currentTime, RakNet::SystemAddress systemAddress, unsigned int &flags, RakNet::BitStream *outBitStream, bool *includeTimestamp )
	{
	...
	ReplicaReturnResult  RakNetChildReplica::SendDestruction(RakNet::BitStream *outBitStream, RakNet::SystemAddress systemAddress, bool *includeTimestamp)
	{
		// Optional, nothing to send here.
		return REPLICA_PROCESSING_DONE;
	}

	ReplicaReturnResult RakNetChildReplica::ReceiveDestruction(RakNet::BitStream *inBitStream, RakNet::SystemAddress systemAddress, RakNet::Time timestamp)
	{
	...
	ReplicaReturnResult RakNetChildReplica::SendScopeChange(bool inScope, RakNet::BitStream *outBitStream, RakNet::Time currentTime, RakNet::SystemAddress systemAddress, bool *includeTimestamp)
	{
		outBitStream->Write(inScope);
		return REPLICA_PROCESSING_DONE;
	}

	ReplicaReturnResult RakNetChildReplica::ReceiveScopeChange(RakNet::BitStream *inBitStream, RakNet::SystemAddress systemAddress, RakNet::Time timestamp)
	{
		return REPLICA_PROCESSING_DONE;
	}

	ReplicaReturnResult RakNetChildReplica::Serialize(bool *sendTimestamp, RakNet::BitStream *outBitStream, RakNet::Time lastSendTime, PacketPriority *priority, PacketReliability *reliability, RakNet::Time currentTime, RakNet::SystemAddress systemAddress, unsigned int &flags)
	{
		// Don't send back to the owner of an object.
		//if (playerId==m_Owner)
		//	return REPLICA_PROCESSING_DONE;
		...
		net_obj->Serialize(sendTimestamp, outBitStream, lastSendTime, priority, reliability, currentTime, RakNet::SystemAddress, flags);
		return REPLICA_PROCESSING_DONE;
	}

	ReplicaReturnResult RakNetChildReplica::Deserialize(RakNet::BitStream *inBitStream, RakNet::Time timestamp, RakNet::Time lastDeserializeTime, RakNet::SystemAddress systemAddress )
	{
		//inBitStream->Read(m_DataToReceive);
		if(m_Owner)
		{
			RakNetNetworkChildComponentPtr net_obj = m_Owner->GetFirstComponentByClass<RakNetNetworkChildComponent>();
			net_obj->Deserialize(inBitStream, timestamp, lastDeserializeTime, RakNet::SystemAddress );
			// If this is a server
			RakNetNetworkSystemPtr raknet = SimEngine::Get().GetSimSystemManager()->GetFirstSystemByClass<RakNetNetworkSystem>();
			if (raknet->IsServer())
			{
				// Synchronisation events should be forwarded to other clients
				//m_Manager->SignalSerializeNeeded(this, RakNet::SystemAddress, true);
			}
		}
