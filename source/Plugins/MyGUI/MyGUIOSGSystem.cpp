...
/*
	...
	MyGUI::OpenGLPlatform* MyGUIOSGSystem::InitializeOpenGLPlatform()
	{
		...
		{
			std::string path = locations[i]->GetPath().GetFullPath();
			//remove last slash
	...
	void MyGUIOSGSystem::OnCameraChanged(CameraChangedEventPtr message)
	{
		...
		OSGCameraPtr osg_cam = GASS_DYNAMIC_PTR_CAST<IOSGCamera>(camera_comp);
	...
	void MyGUIOSGSystem::Update(double delta_time, TaskNode* caller)
	{
		if(mimage)
		{
