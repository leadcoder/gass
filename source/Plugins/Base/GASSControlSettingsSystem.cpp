...
namespace GASS
{
	...
	void ControlSettingsSystem::OnSystemInit()
	{
		...
		Load(FilePath("%GASS_DATA_HOME%/engine/config/input_control_settings.xml").GetFullPath());
	}
			...
			while(control_map)
			{
					...
					inputdevice == DEVICE_MOUSE)
				{
					...
					if(control_map->Attribute("Axis"))//axis
					{
						...
						controller->m_Invert  = 1; //no invert
						if(control_map->Attribute("Invert"))
						{
							std::string value = control_map->Attribute("Invert");
							if(StringUtils::ToLower(value) == "true")
								controller->m_Invert = -1;
