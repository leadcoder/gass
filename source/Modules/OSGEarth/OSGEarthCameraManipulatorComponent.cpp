...
//#include "Modules/Graphics/IOSGNode.h"
...
namespace GASS
{
	...
	void OSGEarthCameraManipulatorComponent::OnInitialize()
	{
		...
		auto earth_sm = GetSceneObject()->GetScene()->GetFirstSceneManagerByClass<OSGEarthGraphicsSceneManager>();
		if (!earth_sm)
			GASS_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, "Failed to find OSGEarthGraphicsSceneManager", "OSGEarthCameraManipulatorComponent::OnInitialize");
		...
		if (!manip)
			GASS_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, "Failed get osgEarth::Util::EarthManipulator from OSGEarthGraphicsSceneManager", "OSGEarthCameraManipulatorComponent::OnInitialize");
		
		osg::ref_ptr<osg::Group> fog_root = earth_sm->GetOSGRootNode();
